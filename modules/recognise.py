    ...
    while True:
        with sr.Microphone() as source:
            try:
                # r.adjust_for_ambient_noise(source, duration=1)  # 適應環境噪音
                r.energy_threshold = 300  # （數值可調整，數值越高代表需要更大聲的輸入才會觸發語音辨識）。    
                debug(f"請說話...{r.energy_threshold:.2f}")      
                audio = r.listen(source,timeout=timeout)  # 設定超時時間
...
def record(seconds):
    ...
    p, frames = microphone_int(seconds)  # 開啟麥克風錄音
...
def microphone_int(seconds):
    ...
    frames = []
    # print(f"開始錄音，最多 {seconds} 秒。按任意鍵可提前結束...")

    start_time = time.time()
    ct=0
    while time.time() - start_time < seconds:
        if msvcrt.kbhit():  #按任意鍵，提前結束錄音。
            msvcrt.getch()  # 清掉按鍵
            break
